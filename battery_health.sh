...
read answer
case $answer in
	"y") echo -n "Hello! Write battery percent(%): (or type 1 for 60 % of charge or 2 for 80 % of charge) "
		read bat_percent
		if [ $bat_percent -ge 1 ]; then
			echo 60 | sudo tee /sys/class/power_supply/BAT?/charge_control_end_threshold
   			...
   			echo $bat_percent | sudo tee /sys/class/power_supply/BAT?/charge_control_end_threshold
		else
   			echo "Something wrong. Please enter a valid max limit between 1 and 100"
		fi;;
	"n") echo "Have a nice day!";;
	*) echo "Command is not correct. Retry again"
esac
